...
// ceMap: ma trận đầu ra
// numBlks: số block mỗi hàng
// blockArr: mảng đánh dấu
__global__ void cumEnergyMapKernel2(int * matrix, int width, int * ceMap,
        int numBlks, volatile int * blockArr)
{
    // lấy chỉ số bi của block thay cho blockIdx
    __shared__ int bi;
    if (threadIdx.x == 0){
    ...
    // Kiểm tra xem 3 block phía dưới đã tính xong chưa
    if (threadIdx.x == 0){
        if (bi >= numBlks){
            int r = bi / numBlks;
            int c = bi % numBlks;
            ...
            while (blockArr[iMid] == 0 || blockArr[iLeft] == 0 || blockArr[iRight] == 0);
        }
    }
    __syncthreads();
    ...
    __syncthreads();
    if (threadIdx.x == 0) // đánh dấu block này đã tính xong
        blockArr[bi] = 1;
}
...
void seamCarvingByDevice(uint8_t * inPixels, int &width, int height, int numstep, uint8_t * &outPixels, int blockSize)
{
    ...
    int * blockArr;
    CHECK(cudaMalloc(&blockArr, height * ((width - 1) / blockSize + 1) * sizeof(int)));
    ...
    for (int itr = 0; itr < numstep; ++itr)
    {
        ...
        int gridSizeSeam = height * ((width - 1) / blockSize + 1); // total number of blocks 
        int numBlks = (width - 1) / blockSize + 1; // num blocks per row
        int zero = 0;
        CHECK(cudaMemcpyToSymbol(bCount, &zero, sizeof(int)));
        CHECK(cudaMemset(blockArr, 0, gridSizeSeam * sizeof(int)));
        
        cumEnergyMapKernel2<<<gridSizeSeam, blockSize>>>(d_sobel, width, d_ceMap, numBlks, blockArr);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());
